...
//
// Usage:
//	power64map [-fmt=format] power64.csv
//
// The known output formats are:
//
//  text (default) - print decoding tree in text form
//  decoder - print decoding tables for the power64asm package
package main
...
func usage() {
	fmt.Fprintf(os.Stderr, "usage: power64map [-fmt=format] power64.csv\n")
	os.Exit(2)
}
...
func printDecoder(p *Prog) {
	...
	fmt.Fprintf(&buf, "package power64asm\n\n")
